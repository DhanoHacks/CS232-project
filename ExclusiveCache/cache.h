...
class CACHE : public MEMORY {
  public:
    ...
    int  check_hit(PACKET *packet),
         ...
         prefetch_line_best_offset(uint64_t ip, uint64_t base_addr, uint64_t pf_addr, int prefetch_fill_level, uint32_t prefetch_metadata),
    ...
    void add_mshr(PACKET *packet),
         ...
         fifo_update(uint32_t set, uint32_t way, uint32_t hit),
    ...
    void back_invalidate(int type ,uint64_t addr, int set, int way);
